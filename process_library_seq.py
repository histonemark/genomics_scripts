...
# To know how many files we have to process we match
# each Sample of the first lane. Samples are called:
# PromoterA12_S12_L001_R1_001.fastq.gz ... PromoterA12_S12_L003_R1_004.fastq.gz

samples = [f for f in os.listdir('.') if
           re.search(r'Promoter[A-H][0-9][1-2]?_S[0-9][1-2]?_L001', f)]
snames = '\n'.join(samples)
sys.stdout.write("The files to be processed are %s:" % snames)
...
for sample in samples:
    ...
    sys.stdout.write('The sample being processed is: %d' % outfname)
